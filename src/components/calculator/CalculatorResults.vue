...
const aviationUnit = ref('Pa') // 'Pa' | 'psi' (dynamic pressure)
const aviationSpeedUnit = ref('m/s') // 'm/s' | 'knots' (EAS/CAS)
...
<template>
  <div v-if="result">
    ...
    <div class="print-only print-header">
      <h2>ISO 2533 Standard Atmosphere</h2>
      <p>Altitude: {{ result.geopotentialAltitude?.meters ?? '—' }} m &nbsp;|&nbsp; Date: {{ new Date().toLocaleDateString() }}</p>
    </div>
          ...
          <template v-if="!collapsedGroups.has(group.group)">
            ...
            <tr>
              ...
              <td class="property-value">
                {{ aviationUnit === 'psi' ? formatValue(aviationProps.dynamicPressure * 1.450377e-4) : formatValue(aviationProps.dynamicPressure) }}
                <span class="property-unit">{{ aviationUnit === 'psi' ? 'psi' : 'Pa' }}</span>
                <span class="property-unit-toggle">
                  <button :class="{ active: aviationUnit !== 'psi' }" @click.stop="aviationUnit = 'Pa'">Pa</button>
                  <button :class="{ active: aviationUnit === 'psi' }" @click.stop="aviationUnit = 'psi'">psi</button>
                </span>
              </td>
            </tr>
            <tr>
              ...
              <td class="property-value">
                {{ aviationSpeedUnit === 'knots' ? formatValue(aviationProps.eas * 1.9438445) : formatValue(aviationProps.eas) }}
                <span class="property-unit">{{ aviationSpeedUnit }}</span>
              </td>
            </tr>
            <tr>
              <td class="property-name"><span class="property-symbol"><math><msub><mi>V</mi><mi>C</mi></msub></math></span> Calibrated Airspeed (ISA)</td>
              <td class="property-value">
                {{ aviationSpeedUnit === 'knots' ? formatValue(aviationProps.cas * 1.9438445) : formatValue(aviationProps.cas) }}
                <span class="property-unit">{{ aviationSpeedUnit }}</span>
                <span class="property-unit-toggle">
                  <button :class="{ active: aviationSpeedUnit === 'm/s' }" @click.stop="aviationSpeedUnit = 'm/s'">m/s</button>
                  <button :class="{ active: aviationSpeedUnit === 'knots' }" @click.stop="aviationSpeedUnit = 'knots'">knots</button>
                </span>
              </td>
            </tr>
          </template>
    ...
    <div class="print-only print-footer">
      Generated by atmospheris.org &nbsp;|&nbsp; {{ typeof window !== 'undefined' ? window.location.href : '' }}
    </div>
  </div>
